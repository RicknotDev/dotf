...
func TestReorganizeAlreadyFormatted(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(dir, "zsh"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "zsh", ".zshrc"), []byte("data"), 0644)

	// Also create layers/ structure already
	_ = os.MkdirAll(filepath.Join(dir, "layers", "shell", "zsh"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "layers", "shell", "zsh", ".zshrc"), []byte("layered data"), 0644)
...
func TestReorganizePreservesOriginals(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "zsh", ".zshrc"), zshContent, 0644)

	_ = os.MkdirAll(filepath.Join(dir, "alacritty", ".config", "alacritty"), 0755)
	alacrittyContent := []byte("[window]\nopacity = 0.95\n")
	_ = os.WriteFile(filepath.Join(dir, "alacritty", ".config", "alacritty", "alacritty.toml"), alacrittyContent, 0644)
...
func TestReorganizeAmbiguityReported(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(dir, "unknown_app"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "unknown_app", "config.conf"), []byte("data"), 0644)

	_ = os.MkdirAll(filepath.Join(dir, "custom_tool"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "custom_tool", "settings.json"), []byte("{}"), 0644)
...
func TestReorganizeOrphanWithFiles(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(dir, "my_app"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "my_app", "config.yml"), []byte("data"), 0644)
	_ = os.WriteFile(filepath.Join(dir, "my_app", "settings.yml"), []byte("more"), 0644)
...
func TestReorganizeFlatStructureWithGitSubmodule(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(dir, "nvim"), 0755)
	// .git is a file for submodules, not a directory
	_ = os.WriteFile(filepath.Join(dir, "nvim", ".git"), []byte("gitdir: ../.git/modules/nvim"), 0644)
	_ = os.WriteFile(filepath.Join(dir, "nvim", "init.lua"), []byte("vim.opt.number=true"), 0644)
	_ = os.MkdirAll(filepath.Join(dir, "nvim", "lua", "plugins"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "nvim", "lua", "plugins", "treesitter.lua"), []byte("return {}"), 0644)
